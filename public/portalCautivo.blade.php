...
<html lang="en">
  <head>
    ...
    <!-- Bootstrap CSS -->
    <link rel="stylesheet" type="text/css" href="../../portal-cautivo1/css/bootstrap.css">
...
<div class="modal fade" id="referenceOxxo" tabindex="-1" aria-labelledby="referenceOxxo" aria-hidden="true">
    ...
    <script>
      ...
      $("#okMsisdn").click(function(){
        ...
        Swal.fire({
          title: 'Estamos trabajando en su petición.',
            ...
            $.ajax({
              ...
              success: function(data){
                ...
                $.ajax({
                  url: 'altan.php',
                  type: 'GET',
                  data: {type:'getStatus', msisdn:msisdn, token:JWT},
                  success: function(data){
                      data = JSON.parse(data);
                      console.log(data);
                    if (data.exists == 1) {
                      ...
                      let dataClient = data.data_client;
                      let client_id = dataClient[0].client_id;
                      let client_name = dataClient[0].client_name;
                      let client_lastname = dataClient[0].client_lastname;
                      let client_email = dataClient[0].client_email;
                      let client_cellphone = dataClient[0].client_cellphone;
                      $.ajax({
                        url: 'altan.php',
                        type: 'GET',
                        data: {type:'getData', msisdn:msisdn, status:status, token:JWT},
                        success:function(data){
                          //extracion de datos
                          data = JSON.parse(data);
                          let type = data.type;
                          let number_id = data.number_id;
                          $('#referencestype_id').val(type);
                          $('#number_id').val(number_id);

                          //validacion para saber type
                          if (type == 5) {
                            let superData = data.superData;
                            superData = JSON.parse(superData);
                            let surplus = superData.packsSurplus;
                            // console.log(superData)
                            // return false;
                            surplus.forEach(function(element){
                              options+= "<option value='"+element.offerID+"' data-offer-id='"+element.id+"' data-rate-price='"+element.price_sale+"' data-rate-id='"+element.id+"' data-offer-id='"+element.alta_offer_id+"'>"+element.name+"</option>"
                            })
                            document.getElementById("ofertas").style.display = "block";
                            
                            $('#alert-rate').removeClass('d-none')
                            $('#alert-rate').html('<strong>Tu plan es: '+superData.dataMSISDN.rate_name+'<br> Tenemos disponibles para ti los siguientes paquetes:</strong>')
                            $('#ofertas').html(options);
                            $('#concepto').val('Compra de GB Altcel')
                            $('#name').val(superData.dataMSISDN.name_user)
                            $('#lastname_user').val(superData.dataMSISDN.lastname_user)
                            $('#email_user').val(superData.dataMSISDN.email_user)
                            $('#cellphone_user').val(superData.dataMSISDN.cellphone_user)
    
                            $('#user_id').val(superData.dataMSISDN.id_user);
                            $('#client_id').val(superData.dataMSISDN.id_user);
                            $('#number_id').val(superData.dataMSISDN.number_id);
                          }else if (type == 1) {
                            let superData = data.superData;
                            superData = JSON.parse(superData);
                            let payment = superData.payment;
                            let amount1 = superData.payment[0].amount;
                            let offerId = superData.payment[0].offer_id;
                            let payId = superData.payment[0].pay_id;
                            $('#pay_id').val(payId);
                            payment.forEach(function(element){
                              options+= "<option value='"+element.offer_id+"' data-offer-id='"+element.offer_id+"' data-activation-id='"+element.activation_id+"' data-rate-price='"+element.amount+"' data-pay-id='"+element.pay_id+"' data-rate-id='"+element.rate_id+"' data-status-payment='"+element.status_payment+"'>"+element.rate_name+"</option>"
                            })
                            document.getElementById("ofertas").style.display = "block";

                            $('amount_view').val(amount1);
                            $('#ofertas').html(options);
                            $('#name').val(client_name)
                            $('#lastname_user').val(client_lastname)
                            $('#email_user').val(client_email)
                            $('#cellphone_user').val(client_cellphone)  
                            $('#offer_id').val(offerId)
                            $('#concepto').val('Reactivación de servicio Altcel2')
