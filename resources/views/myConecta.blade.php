...
<div class="row no-gutters" style="justify-content: center; z-index:999 !important;">
    <div class="col-md-7 mov_top" style="justify-content: center;">
        ...
        @endif
            <div class="mov_form__regpag">
                <h2 class="mb-4 text__recarga">RECARGAS Y PAGOS DE MENSUALIDAD</h2>
                <h3 class="mb-4 text__recarga">INGRESA TU SIM</h3>
                <form method="POST" id="contactForm" name="contactForm" class="contactForm">
                    <div class="row">
                        <div class="col-md-12">
                        <!-- <div class="col-md-6"> -->
                            <div class="form-group">
                                <label class="label text__label" for="name">Para realizar una recarga o el pago de mensualidad, ingresa tu número</label>
                                <input type="tel" class="form-control inputRecarg" name="msisdn" id="msisdn" placeholder="3339061234" autocomplete="off" maxlength="10" style="text-align: center; font-size: 30px;">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <!-- <div class="col-md-6"> -->
                        <div class="col-md-12">
                            <label class="label text__label" for="name">Si no conoces tu número, puedes comunicarte con nosotros al 800 2 ALTCEL(258235)</label>
                        </div>
                    </div>
                ...
                <h2 class="marck__logo">
                    ...
                    <div>
                        <img src="{{ asset('plantilla/images/walmart.png') }}" width="100px" alt="">
                    </div> 
                    <div>
                        <img src="{{ asset('plantilla/images/aurrera.png') }}" width="75px" alt="">
                    </div>
                    <div>
                        <img src="{{ asset('plantilla/images/waldos.png') }}" width="75px" alt="">
                    </div>
                    <div>
...
<section class="ftco-section bg-light py-2 d-none" id="surplusRatesContent">
    <div class="container container__conecta">
        <center>
...
<section class="ftco-section bg-light py-2 d-none" id="informationMSISDN">
    <div class="container containe_pag__mensual">
...
<section class="ftco-section bg-light py-2 d-none" id="paymentContent">
    <div class="container containerConting">
        <div class="contact-wrap w-100 p-md-5 p-4">
            <center><h3 class="mb-4">ELIGE EL MÉTODO DE PAGO</h3></center>
            <center>
            <div class="alert alert-warning" role="alert">
                ATENCIÓN!! A continuación te mostramos diversos pagos con tarjeta, por si llegas a tener problemas con tu pago, hemos habilitado más de una opción.
            </div>
            </center>
            <div class="row" style="justify-content: center;">
                <button class="btn btn-primary mr-2 paymentMethod mt-1 mb-1" data-type="reference" id="btn-reference-payment-way">REFERENCIA</button>
                <button class="btn btn-primary mr-2 paymentMethod mt-1 mb-1" data-type="card" id="btn-card-payment-way">TARJETA con CONEKTA</button>
                <button class="btn btn-primary mr-2 paymentMethod mt-1 mb-1" data-type="cardStripe" id="btn-card-payment-way">TARJETA con STRIPE</button>
            </div>
        </div>
    </div>
</section>
...
<section class="ftco-section bg-light py-2 d-none" id="referenceContent">
    <div class="container">
        <div class="row row_marg" style="justify-content: center;">
		...
		<input type="hidden" id="offerAltan">
        ...
        <input type="hidden" id="data_rate_name">
        <input type="hidden" id="number_to_recharge">
...
<div class="modal fade" id="emailFixModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="emailFixTitle"></h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body" >
          <label class="label" id="emailFixText" style="color: #00bd56; font-weight:600;">Para poder contactarnos contigo en caso de algún problema con tu pago, te pedimos nos regales tu email.</label>
        <div class="col-md-6"> 
            <div class="form-group">
                <label class="label" for="emailFix">Email</label>
                <input type="email" class="form-control" name="emailFix" id="emailFix" placeholder="juanito@mail.com">
            </div>
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-primary" id="saveEmailFixed">Guardar</button>
      </div>
    </div>
  </div>
</div>
    ...
    //    return false;
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                var dest = $("#surplusRatesContent").offset().top;
                    ...
                    ratesChange+='<div class="col-md-6 col-lg-3 ">'+
                                    '<div class="staff" >'+
                                        '<div class="text pt-3 px-3 pb-4 text-center">'+
                                            ...
                                            '<span class="position mb-2 animate__animated animate__rubberBand animate__infinite">$'+parseFloat(element.rate_price).toFixed(2)+'</span>'+
                                            '<div class="faded">'+
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                if(http_code != 5){
                    ...
                    var dest = $("#informationMSISDN").offset().top;
                    $("html, body").animate({scrollTop: dest},600);
...
function defineType(id){
    ...
    let rate_name = $('#'+id).data('name');
    let offerAltan = $('#'+id).data('offer-altan');
    let msisdn = $('#msisdn').val();
    let bool_mov = $('#bool_mov').val();
    ...
    $('#offerAltan').val(offerAltan);
    $('#rate_id').val(rate_id);
    $('#pay_id').val(pay_id);
    $('#referencestype').val(referencestype);
    $('#concepto').val(concepto);
    $('#pack_id').val(pack_id);
    $('#data_rate_name').val(rate_name);
    $('#number_to_recharge').val(msisdn);
...
$('.paymentMethod').click(function(){
    ...
    let emailFix = $('#email').val();
    let nameFix = $('#name').val();
    let lastnameFix = $('#lastname').val();

    emailRegex = /^[-\w.%+]{1,64}@(?:[A-Z0-9-]{1,63}\.){1,125}[A-Z]{2,63}$/i;
    if (emailRegex.test(emailFix)) {
        
    } else {
        $('#emailFixTitle').html('Hola, '+nameFix+' '+lastnameFix);
        $('#emailFixModal').modal('show');
        return false;
    } 
    ...
    }else if(method == 'cardStripe'){
        let amount = $('#amount').val();
        let offer_id = $('#offer_id').val();
        let offerAltan = $('#offerAltan').val();
        let rate_id = $('#rate_id').val();
        let referencestype = $('#referencestype').val();
        let number_id = $('#number_id').val();
        let client_id = $('#client_id').val();
        let pay_id = $('#pay_id').val();
        let exists = $('#exists').val();
        let pack_id = $('#pack_id').val();
        let rate_name = $('#data_rate_name').val();
        let msisdn = $('#number_to_recharge').val();

        $('#STRIPE_amount').val(amount);
        $('#STRIPE_offer_id').val(offer_id);
        $('#STRIPE_offerAltan').val(offerAltan);
        $('#STRIPE_rate_id').val(rate_id);
        $('#STRIPE_referencestype').val(referencestype);
        $('#STRIPE_number_id').val(number_id);
        $('#STRIPE_client_id').val(client_id);
        $('#STRIPE_pay_id').val(pay_id);
        $('#STRIPE_exists').val(exists);
        $('#STRIPE_pack_id').val(pack_id);
        $('#STRIPE_data_rate_name').val(rate_name);
        $('#STRIPE_number_to_recharge').val(msisdn);
...
$('#saveEmailFixed').click(function(){
    let email = $('#emailFix').val();

    emailRegex = /^[-\w.%+]{1,64}@(?:[A-Z0-9-]{1,63}\.){1,125}[A-Z]{2,63}$/i;
    if (emailRegex.test(email)) {
        $('#email').val(email);
        $('#emailFixModal').modal('hide');
        Swal.fire({
            icon: 'success',
            title: 'Well done!!',
            text: 'Puedes continuar con tu pago.',
            timer: 2000
        });
    } else {
        Swal.fire({
            icon: 'error',
            title: 'Oops...',
            text: 'Ingrese un email válido, por favor.',
        });
    } 
});
...
<script>
    ...
    var stripe = Stripe("pk_live_51LSOeLHo1IBUssic4hcrlM7sJzm78FfV2YwZGOfhPBKQrHloVqU3E7jSvRJnhNIIzdFIhv0LDWkDZcaU8GkmfXFJ000Msl2iQ2");
